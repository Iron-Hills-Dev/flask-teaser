...
def is_valid_data_folder(_path: str) -> bool:
    logging.debug("Checking if data folder is valid")
    _real_path = path.join(_path, ".car_file_data_structure")
    if path.exists(_real_path):
        logging.debug("Folder given is valid data folder - checking version")
        ...
        logging.warning("Folder chosen is full and it is not a data folder - returning False")
        return False
...
def init_data_structure(_path: str):
    ...
    try:
        ...
        if is_valid_data_folder(_path):
            ...
            logging.critical("ABORTING INIT - Folder with car data is not empty or it is outdated")
            exit(1)
...
def create_data_structure(_path: str) -> bool:
    ...
    logging.debug("Created data structure")
    return True
